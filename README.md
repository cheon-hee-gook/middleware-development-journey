...
### **구현된 미들웨어**
1. **요청 제한**:
   - 클라이언트의 요청 빈도를 제한하여 서비스 남용 방지 
   - Redis를 활용하여 요청 횟수를 저장하고, 설정된 시간 창(Window) 동안 요청 제한 
   - 제한 초과 시 HTTP 429 응답 반환 
   - 요청 제한 로직:
     - 클라이언트 IP 기반으로 요청 제한 
     - Redis 키-값 구조를 사용하여 IP별 요청 횟수 저장 
     - 설정된 요청 횟수를 초과한 경우 제한 응답 반환 
     - 제한 시간(예: 60초) 이후 요청 카운터 초기화
...
### **학습 포인트**
1. Redis와 FastAPI의 연동:
   - Redis를 통해 요청 빈도를 관리 
   - 클라이언트의 요청 데이터를 Redis 키-값 구조로 저장하고 관리
2. API 요청 제한의 중요성:
   - 서비스 남용 방지 
   - 설정된 요청 횟수를 초과한 클라이언트는 제한 응답 반환
3. Postman을 활용한 테스트:
   - 정상 요청과 초과 요청 시나리오를 검증 
   - 상태 코드 및 응답 메시지 확인

## **미들웨어 공부 후기**
이번 스터디를 통해 FastAPI와 Starlette 기반의 미들웨어 설계와 구현 원리를 깊이 이해할 수 있었습니다. 특히, 요청 상태 관리, 데이터 암호화/복호화, 요청 제한 등의 다양한 요구사항을 해결하며, 실무에서 활용 가능한 유용한 패턴을 익혔습니다. 무엇보다 삽질 과정에서 얻은 교훈들이 앞으로의 개발에 큰 자산이 될 것 같습니다! 